...
layout (std430, binding = 30) readonly buffer InstanceDataLightingMode
{
    int u_LightingMode[];
};

layout (location = 0) out flat int v_MaterialIndex;
layout (location = 1) out flat int v_LightingMode;
...
void main()
{
    ...
    v_LightingMode                   = u_LightingMode[gl_InstanceID];
...
layout (location = 1) in flat int v_LightingMode_in[];
...
void main()
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        v_LightingMode           = v_LightingMode_in[i];
...
layout(location = 0) out int o_LightingMode;
...
layout (location = 1) in flat int v_LightingMode;
