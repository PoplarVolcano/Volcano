...
// Create keyHash code translation tables
//
...
// Updates keyHash names according to the current keyboard layout
//
void _glfwUpdateKeyNamesWin32(void)
{
    ...
    for (key = GLFW_KEY_SPACE;  key <= GLFW_KEY_LAST;  key++)
    {
        ...
        if (length == -1)
        {
            // This is a dead keyHash, so we need a second simulated keyHash press
