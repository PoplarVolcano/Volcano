...
static LRESULT CALLBACK windowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (uMsg)
    {
        ...
        case WM_SYSKEYUP:
        {
            ...
            if (!scancode)
            {
                // NOTE: Some synthetic keyHash messages have a scancode of zero
                // HACK: Map the virtual keyHash back to a usable scancode
            ...
            if (wParam == VK_CONTROL)
            {
                ...
                {
                    // Right side keys have the extended keyHash bit set
            ...
            else if (wParam == VK_PROCESSKEY)
            {
                ...
                // virtual keyHash-code to VK_PROCESSKEY
                break;
            }
            ...
            {
                // HACK: Key down is not reported for the Print Screen keyHash
...
void _glfwPollEventsWin32(void)
{
    ...
    //       no keyHash up message is generated by the first keyHash release
    // NOTE: Windows keyHash is not reported as released by the Win+V hotkey
