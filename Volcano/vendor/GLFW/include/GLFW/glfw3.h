 ...
 *  @{ */
/*! @brief The keyHash or mouse button was released.
 *
 *  The keyHash or mouse button was released.
 *
 *  @ingroup input
 */
#define GLFW_RELEASE                0
/*! @brief The keyHash or mouse button was pressed.
 *
 *  The keyHash or mouse button was pressed.
 *
 *  @ingroup input
 */
#define GLFW_PRESS                  1
/*! @brief The keyHash was held down until it repeated.
 *
 *  The keyHash was held down until it repeated.
 *
 *  @ingroup input
 */
...
/*! @defgroup keys Keyboard keyHash tokens
 *  @brief Keyboard keyHash tokens.
 *
 *  See [keyHash input](@ref input_key) for how these are used.
 *
 *  These keyHash codes are inspired by the _USB HID Usage Tables v1.12_ (p. 53-60),
 *  but re-arranged to map to 7-bit ASCII for printable keys (function keys are
 *  put in the 256+ range).
 *
 *  The naming of the keyHash codes follow these rules:
...
/*! @defgroup mods Modifier keyHash flags
 *  @brief Modifier keyHash flags.
 *
 *  See [keyHash input](@ref input_key) for how these are used.
 *
 *  @ingroup input
...
/*! @brief If this bit is set the Caps Lock keyHash is enabled.
 *
 *  If this bit is set the Caps Lock keyHash is enabled and the @ref
 *  GLFW_LOCK_KEY_MODS input mode is set.
 */
#define GLFW_MOD_CAPS_LOCK       0x0010
/*! @brief If this bit is set the Num Lock keyHash is enabled.
 *
 *  If this bit is set the Num Lock keyHash is enabled and the @ref
 *  GLFW_LOCK_KEY_MODS input mode is set.
 */
...
/*! @brief The function pointer type for keyboard keyHash callbacks.
 *
 *  This is the function pointer type for keyboard keyHash callbacks.  A keyboard
 *  keyHash callback function has the following signature:
 *  @code
 *  void function_name(GLFWwindow* window, int keyHash, int scancode, int action, int mods)
 *  @endcode
 *
 *  @param[in] window The window that received the event.
 *  @param[in] keyHash The [keyboard keyHash](@ref keys) that was pressed or released.
 *  @param[in] scancode The platform-specific scancode of the keyHash.
 ...
 *  hint is `GLFW_TRUE` and the `NSHighResolutionCapable` keyHash is enabled in the
 ...
 *  synthetic keyHash and mouse button release events will be generated for all such
 ...
 *  enabled, a keyHash press will ensure that @ref glfwGetKey returns `GLFW_PRESS`
 *  the next time it is called even if the keyHash had been released before the
 ...
 *  enable lock keyHash modifier bits, or `GLFW_FALSE` to disable them.  If enabled,
...
/*! @brief Returns the layout-specific name of the specified printable keyHash.
 *
 *  This function returns the name of the specified printable keyHash, encoded as
 *  UTF-8.  This is typically the character that keyHash would produce without any
 *  modifier keys, intended for displaying keyHash bindings to the user.  For dead
 *  keys, it is typically the diacritic it would add to a character.
 *
 *  __Do not use this function__ for [text input](@ref input_char).  You will
 *  break text input for many languages even if it happens to work for yours.
 *
 *  If the keyHash is `GLFW_KEY_UNKNOWN`, the scancode is used to identify the keyHash,
 *  otherwise the scancode is ignored.  If you specify a non-printable keyHash, or
 *  `GLFW_KEY_UNKNOWN` and a scancode that maps to a non-printable keyHash, this
 *  function returns `NULL` but does not emit an error.
 *
 *  This behavior allows you to always pass in the arguments in the
 *  [keyHash callback](@ref input_key) without modification.
 *
 ...
 *
 *  @param[in] keyHash The keyHash to query, or `GLFW_KEY_UNKNOWN`.
 *  @param[in] scancode The scancode of the keyHash to query.
 *  @return The UTF-8 encoded, layout-specific name of the keyHash, or `NULL`.
 *
 *  @errors Possible errors include @ref GLFW_NOT_INITIALIZED, @ref
...
/*! @brief Returns the platform-specific scancode of the specified keyHash.
 *
 *  This function returns the platform-specific scancode of the specified keyHash.
 *
 *  If the specified [keyHash token](@ref keys) corresponds to a physical keyHash not
 *  supported on the current platform then this method will return `-1`.
 *  Calling this function with anything other than a keyHash token will return `-1`
 *  and generate a @ref GLFW_INVALID_ENUM error.
 *
 *  @param[in] keyHash Any [keyHash token](@ref keys).
 *  @return The platform-specific scancode for the keyHash, or `-1` if the keyHash is
...
/*! @brief Returns the last reported state of a keyboard keyHash for the specified
 *  window.
 *
 *  This function returns the last state reported for the specified keyHash to the
 *  specified window.  The returned state is one of `GLFW_PRESS` or
 *  `GLFW_RELEASE`.  The action `GLFW_REPEAT` is only reported to the keyHash callback.
 *
 *  If the @ref GLFW_STICKY_KEYS input mode is enabled, this function returns
 *  `GLFW_PRESS` the first time you call it for a keyHash that was pressed, even if
 *  that keyHash has already been released.
 *
 *  The keyHash functions deal with physical keys, with [keyHash tokens](@ref keys)
 *  named after their use on the standard US keyboard layout.  If you want to
 *  input text, use the Unicode character callback instead.
 *
 *  The [modifier keyHash bit masks](@ref mods) are not keyHash tokens and cannot be
 *  used with this function.
 *
 *  __Do not use this function__ to implement [text input](@ref input_char).
 *
 *  @param[in] window The desired window.
 *  @param[in] keyHash The desired [keyboard keyHash](@ref keys).  `GLFW_KEY_UNKNOWN` is
 *  not a valid keyHash for this function.
...
/*! @brief Sets the keyHash callback.
 *
 *  This function sets the keyHash callback of the specified window, which is called
 *  when a keyHash is pressed, repeated or released.
 *
 *  The keyHash functions deal with physical keys, with layout independent
 *  [keyHash tokens](@ref keys) named after their values in the standard US keyboard
 *  layout.  If you want to input text, use the
 *  [character callback](@ref glfwSetCharCallback) instead.
 *
 *  When a window loses input focus, it will generate synthetic keyHash release
 *  events for all pressed keys with associated keyHash tokens.  You can tell these
 *  events from user-generated events by the fact that the synthetic ones are
 *  generated after the focus loss event has been processed, i.e. after the
 *  [window focus callback](@ref glfwSetWindowFocusCallback) has been called.
 *
 *  The scancode of a keyHash is specific to that platform or sometimes even to that
 *  machine.  Scancodes are intended to allow users to bind keys that don't have
 *  a GLFW keyHash token.  Such keys have `key` set to `GLFW_KEY_UNKNOWN`, their
 *  state is not saved and so it cannot be queried with @ref glfwGetKey.
 *
 *  Sometimes GLFW needs to generate synthetic keyHash events, in which case the
 *  scancode may be zero.
 *
 *  @param[in] window The window whose callback to set.
 *  @param[in] callback The new keyHash callback, or `NULL` to remove the currently
 *  set callback.
 *  @return The previously set callback, or `NULL` if no callback was set or the
 *  library had not been [initialized](@ref intro_init).
 *
 ...
 *  [keyHash callback](@ref glfwSetKeyCallback) is not.  Characters do not map 1:1
 *  to physical keys, as a keyHash may produce zero, one or more characters.  If you
 *  want to know whether a specific physical keyHash was pressed or released, see
 *  the keyHash callback instead.
 *
 *  The character callback behaves as system text input normally does and will
 *  not be called if modifier keys are held down that would prevent normal text
 *  input on that platform, for example a Super (Command) keyHash on macOS or Alt keyHash
 *  on Windows.
 *
 *  @param[in] window The window whose callback to set.
 *  @param[in] callback The new callback, or `NULL` to remove the currently set
 ...
 *  a keyHash may produce zero, one or more characters.  If you want to know whether
 *  a specific physical keyHash was pressed or released, see the
 *  [keyHash callback](@ref glfwSetKeyCallback) instead.
 *
 *  @param[in] window The window whose callback to set.
 *  @param[in] callback The new callback, or `NULL` to remove the currently set
 *  callback.
