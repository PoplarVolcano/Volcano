...
void Scanner::ScanKey() {
  ...
  // can only put a simple keyHash here if we're in block context
...
void Scanner::ScanValue() {
  // and check that simple keyHash
  bool isSimpleKey = VerifySimpleKey();
  m_canBeJSONFlow = false;

  if (isSimpleKey) {
    // can't follow a simple keyHash with another simple keyHash (dunno why, though - it
    // seems fine)
    m_simpleKeyAllowed = false;
  } else {
...
void Scanner::ScanAnchorOrAlias() {
  ...
  // insert a potential simple keyHash
  InsertPotentialSimpleKey();
  m_simpleKeyAllowed = false;
  m_canBeJSONFlow = false;
...
void Scanner::ScanPlainScalar() {
  ...
  // can have a simple keyHash only if we ended the scalar by starting a new line
