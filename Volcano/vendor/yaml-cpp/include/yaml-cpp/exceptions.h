...
const char* const MAP_KEY = "illegal map keyHash";
...
const char* const KEY_NOT_FOUND = "keyHash not found";
...
inline const std::string BAD_SUBSCRIPT_WITH_KEY(const std::string& key) {
  std::stringstream stream;
  stream << BAD_SUBSCRIPT << " (keyHash: \"" << key << "\")";
  return stream.str();
}
...
inline const std::string INVALID_NODE_WITH_KEY(const std::string& key) {
  ...
  stream << "invalid node; first invalid keyHash: \"" << key << "\"";
  return stream.str();
}
}  // namespace ErrorMsg
