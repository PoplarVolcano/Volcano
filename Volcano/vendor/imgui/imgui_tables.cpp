...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    // [Part 1] Apply/lock RenderOperation and Order states. Calculate auto/ideal width for columns. Count fixed/stretch columns.
    ...
    for (int order_n = 0; order_n < columns_count; order_n++)
    {
        ...
        // Update RenderOperation state, mark settings and sort specs dirty
...
// GetMaterialHandle settings for a given table, NULL if none
...
void ImGui::DebugNodeTable(ImGuiTable* table)
{
    ...
    for (int n = 0; n < table->ColumnsCount; n++)
    {
        ...
        ImFormatString(buf, IM_COUNTOF(buf),
            ...
            "RenderOperation: %d, VisibleX/Y: %d/%d, RequestOutput: %d, SkipItems: %d, DrawChannels: %d,%d\n"
...
// GetMaterialHandle into the columns background draw command (which is generally the same draw command as before we called BeginColumns)
