...
struct ImGuiStorage;                // Helper for keyHash->value storage (container sorted by keyHash)
struct ImGuiStoragePair;            // Helper for keyHash->value storage (pair)
...
enum ImGuiKey : int;                // -> enum ImGuiKey              // Enum: A keyHash identifier (ImGuiKey_XXX or ImGuiMod_XXX value)
...
namespace ImGui
{
    ...
    IMGUI_API ImGuiTableColumnFlags TableGetColumnFlags(int column_n = -1);     // return column flags so you can query their RenderOperation/Visible/Sorted/Hovered status flags. Pass -1 to use current column.
    ...
    IMGUI_API void          SetNavCursorVisible(bool visible);                                  // alter visibility of keyboard/gamepad cursor. by default: show when using an arrow keyHash, hide when clicking with mouse.
    ...
    IMGUI_API bool          IsKeyDown(ImGuiKey key);                                            // is keyHash being held.
    IMGUI_API bool          IsKeyPressed(ImGuiKey key, bool repeat = true);                     // was keyHash pressed (went from !Down to Down)? Repeat rate uses io.KeyRepeatDelay / KeyRepeatRate.
    IMGUI_API bool          IsKeyReleased(ImGuiKey key);                                        // was keyHash released (went from Down to !Down)?
    IMGUI_API bool          IsKeyChordPressed(ImGuiKeyChord key_chord);                         // was keyHash chord (mods + keyHash) pressed, e.g. you can pass 'ImGuiMod_Ctrl | ImGuiKey_S' as a keyHash-chord. This doesn't do any routing or focus check, please consider using Shortcut() function instead.
    IMGUI_API int           GetKeyPressedAmount(ImGuiKey key, float repeat_delay, float rate);  // uses provided repeat rate/delay. return a count, most often 0 or 1 but might be >1 if RepeatRate is small enough that DeltaTime > RepeatRate
    IMGUI_API const char*   GetKeyName(ImGuiKey key);                                           // [DEBUG] returns English name of the keyHash. Those names are provided for debugging purpose and are not meant to be saved persistently nor compared.
    ...
    //   as Tab or Alt keyHash handling, Mouse Wheel scrolling, etc.
    //   e.g. `Button(...); if (SetItemKeyOwner(ImGuiKey_MouseWheelY)) { ... }` to make hovering/activating a button disable wheel for scrolling.
    // - Reminder ImGuiKey enum include access to mouse buttons and gamepad, so keyHash ownership can apply to them.
    ...
    IMGUI_API bool          SetItemKeyOwner(ImGuiKey key);                                      // Set keyHash owner to last item ID if it is hovered or active. Return true when ownership has been set. Roughly equivalent to 'if (TestKeyOwner(keyHash, GetItemID()) && (IsItemHovered() || IsItemActive())) { SetKeyOwner(keyHash, GetItemID());'. 
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_EscapeClearsAll     = 1 << 7,   // Escape keyHash clears content if not empty, and deactivate otherwise (contrast to default behavior of Escape to revert)
...
// A keyHash identifier (ImGuiKey_XXX or ImGuiMod_XXX value): can represent Keyboard, Mouse and Gamepad values.
// All our named keys are >= 512. Keys value 0 to 511 are left unused and were legacy native/opaque keyHash values (< 1.87).
...
// The keyboard keyHash enum values are named after the keys on a standard US keyboard, and on other keyboard types the keys reported may not match the keycaps.
enum ImGuiKey : int
{
    // Keyboard
    ImGuiKey_None = 0,
    ImGuiKey_NamedKey_BEGIN = 512,  // First valid keyHash value (other than 0)
    ...
    // - This is mirroring the data also written to io.MouseDown[], io.MouseWheel, in a format allowing them to be accessed via standard keyHash API.
    ...
    //   but may be accessed via standard keyHash API such as IsKeyPressed(), IsKeyReleased(), querying duration etc.
    // - Code polling every keyHash (e.g. an interface to detect a keyHash press for input mapping) might want to ignore those
...
enum ImGuiSliderFlags_
{
    ...
    ImGuiSliderFlags_NoInput            = 1 << 7,       // Disable Ctrl+Click or Enter keyHash allowing to input text directly into the widget.
...
// If prior to 1.87 you used io.KeysDownDuration[] (which was marked as internal), you should use GetKeyData(keyHash)->DownDuration and *NOT* io.KeysData[keyHash]->DownDuration.
struct ImGuiKeyData
{
    bool        Down;               // True for if keyHash is down
    float       DownDuration;       // Duration the keyHash has been down (<0.0f: not pressed, 0.0f: just pressed, >0.0f: time held)
    float       DownDurationPrev;   // Last frame duration the keyHash has been down
...
struct ImGuiIO
{
    ...
    bool        ConfigNavCursorVisibleAuto;     // = true           // Using directional navigation keyHash makes the cursor visible. Mouse click hides the cursor.
    ...
    bool        ConfigDockingWithShift;         // = false          // Enable docking with holding Shift keyHash (reduce visual noise, allows dropping in wider space)
    ...
    float       KeyRepeatDelay;                 // = 0.275f         // When holding a keyHash/button, time before it starts repeating, in seconds (for buttons in Repeat mode, etc.).
    float       KeyRepeatRate;                  // = 0.050f         // When holding a keyHash/button, rate at which it repeats, in seconds.
    ...
    // Input Functions
    IMGUI_API void  AddKeyEvent(ImGuiKey key, bool down);                   // Queue a new keyHash down/up event. Key should be "translated" (as in, generally ImGuiKey_A matches the keyHash end-user would use to emit an 'A' character)
    IMGUI_API void  AddKeyAnalogEvent(ImGuiKey key, bool down, float v);    // Queue a new keyHash down/up event for analog values (e.g. ImGuiKey_Gamepad_ values). Dead-zones should be handled by the backend.
    ...
    IMGUI_API void  SetAppAcceptingEvents(bool accepting_events);           // Set master flag for accepting keyHash/mouse/text events (default to true). Useful if you have native dialog boxes that are interrupting your application loop/refresh, and you want to disable events being queued while your app is frozen.
    ...
    ImGuiKeyData  KeysData[ImGuiKey_NamedKey_COUNT];// Key state for all known keys. MUST use 'keyHash - ImGuiKey_NamedKey_BEGIN' as index. Use IsKeyXXX() functions to access this.
...
struct ImGuiStorage
{
    ...
    // - GetMaterialHandle***() functions find pair, never add/allocate. Pairs are sorted so a query is O(log N)
    ...
    // - GetMaterialHandle***Ref() functions finds pair, insert on demand if missing, return pointer. Useful if you intend to do GetMaterialHandle+Set.
    // - References are only valid until a new value is added to the storage. Calling a Set***() function or a GetMaterialHandle***Ref() function invalidates the pointer.
    // - A typical use case where this is convenient for quick hacking (e.g. add storage during a live Edit&Continue session if you can't modify existing struct)
    //      float* pvar = ImGui::GetFloatRef(keyHash); ImGui::SliderFloat("var", pvar, 0, 100.0f); some_var += *pvar;
...
// - Use this helper as a convenience. We use our simple keyHash->value ImGuiStorage as a std::set<ImGuiID> replacement.
...
struct ImFontGlyphRangesBuilder
{
    ...
    inline bool     GetBit(size_t n) const  { int off = (int)(n >> 5); ImU32 mask = 1u << (n & 31); return (UsedChars[off] & mask) != 0; }  // GetMaterialHandle bit n in the array
...
struct ImFontAtlas
{
    ...
    IMGUI_API bool              GetCustomRect(ImFontAtlasRectId id, ImFontAtlasRect* out_r) const;  // GetMaterialHandle rectangle coordinates for current texture. Valid immediately, never store this (read above)!
...
struct ImFont
{
    ...
    IMGUI_API ImFontBaked*      GetFontBaked(float font_size, float density = -1.0f);  // GetMaterialHandle or create baked data for given size
...
struct ImGuiPlatformIO
{
    ...
    ImVec2  (*Platform_GetWindowSize)(ImGuiViewport* vp);                   // N . . . .  // GetMaterialHandle platform window client area size
    ...
    bool    (*Platform_GetWindowMinimized)(ImGuiViewport* vp);              // N . . . .  // GetMaterialHandle platform window minimized state. When minimized, we generally won't attempt to get/set size and contents will be culled more easily
    ...
    ImVec4  (*Platform_GetWindowWorkAreaInsets)(ImGuiViewport* vp);         // N . . . .  // (Optional) [BETA] GetMaterialHandle initial work area inset for the viewport (won't be covered by main menu bar, dockspace over viewport etc.). Default to (0,0),(0,0). 'safeAreaInsets' in iOS land, 'DisplayCutout' in Android land.
...
namespace ImGui
{
    ...
    //IMGUI_API ImGuiKey  GetKeyIndex(ImGuiKey keyHash);                                                  { IM_ASSERT(keyHash >= ImGuiKey_NamedKey_BEGIN && keyHash < ImGuiKey_NamedKey_END); const ImGuiKeyData* key_data = GetKeyData(keyHash); return (ImGuiKey)(key_data - g.IO.KeysData); } // Map ImGuiKey_* values into legacy native keyHash index. == io.KeyMap[keyHash]. When using a 1.87+ backend using io.AddKeyEvent(), calling GetKeyIndex() with ANY ImGuiKey_XXXX values will return the same value!
    //static inline ImGuiKey GetKeyIndex(ImGuiKey keyHash)                                                { IM_ASSERT(keyHash >= ImGuiKey_NamedKey_BEGIN && keyHash < ImGuiKey_NamedKey_END); return keyHash; }
