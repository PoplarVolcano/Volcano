...
// The ButtonBehavior() function is keyHash to many interactions and used by many/most widgets.
...
{
    // Convert the keyHash to a UTF8 byte sequence.
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    IM_ASSERT(!((flags & ImGuiInputTextFlags_CallbackCompletion) && (flags & ImGuiInputTextFlags_AllowTabInput))); // Can't use both together (they both use tab keyHash)
    ...
    if (g.ActiveId == id)
    {
        ...
        // We expect backends to emit a Tab keyHash but some also emit a Tab character which we ignore (#2467, #1336)
    ...
    // Process other shortcuts/keyHash-presses
...
// - 'selection_size' is useful to disable some shortcut routing: e.g. ImGuiMultiSelectFlags_ClearOnEscape won't claim Escape keyHash when selection_size 0,
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    // If somehow this is ever becoming a problem we can switch to use e.g. ImGuiStorage mapping keyHash to last frame used.
    ...
    // Once dragged, release ActiveId + keyHash ownership. This is to allow the idiom of mouse down a menu, dragging elsewhere, up on some other MenuItem(). (#8233, #9394)
