...
namespace Volcano {
	...
	Ref<Shader> Shader::Create(const std::string& filepath, ShaderBackend shaderBackend)
	{
		switch (RendererAPI::Current())
		{
		case RendererAPIType::None:   VOL_CORE_ASSERT(false, "Shader：API为None不支持"); return nullptr;
		case RendererAPIType::OpenGL: return std::make_shared <OpenGLShader>(filepath, shaderBackend);
		}
	...
	Ref<Shader> ShaderLibrary::Load(const std::string filepath, ShaderBackend shaderBackend)
	{
		auto shader = Shader::Create(filepath, shaderBackend);
		Add(shader);
		return shader;
	}
