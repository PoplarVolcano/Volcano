...
namespace Volcano {
    ...
    void Shadow::Init()
    {
        ...
        s_ShadowData.VertexArray = VertexArray::Create();
        Ref<VertexBuffer> vb = VertexBuffer::Create(shadowVertices, sizeof(shadowVertices));
        vb->SetLayout({
            { ShaderDataType::Float3, "a_Position" },
            { ShaderDataType::Float3, "a_Normal"   },
            { ShaderDataType::Float2, "a_TexCoords"}
            });
        s_ShadowData.VertexArray->AddVertexBuffer(vb);
    ...
    void Shadow::BeginScene()
    {
    }

    void Shadow::EndScene()
    {
    }
