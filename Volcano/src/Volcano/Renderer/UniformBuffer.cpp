...
namespace Volcano {
	...
	void UniformBufferManager::Init()
	{
		...
		UniformBufferManager::m_UniformBuffers["DirectionalLight"]            = UniformBuffer::Create((4 + 4 + 4 + 4 + 4) * sizeof(float), 2);
		UniformBufferManager::m_UniformBuffers["PointLight"]                  = UniformBuffer::Create((4 + 4 + 4 + 3 + 1 + 1 + 1) * sizeof(float), 3); // 最多4个点光源
