...
#define VOL_ASSERT_NO_MESSAGE(m_Condition) { if(!(m_Condition)) { VOL_ERROR("Assertion Failed"); __debugbreak(); } }
#define VOL_ASSERT_MESSAGE(m_Condition, ...) { if(!(m_Condition)) { VOL_ERROR("Assertion Failed: {0}", __VA_ARGS__); __debugbreak(); } }
