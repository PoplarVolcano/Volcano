...
#include "ImGuizmo.h"
#include "Volcano/ImGui/ImGuiContextManager.h"
...
namespace Volcano {
	...
	void ImGuiLayer::OnAttach() {
		...
		// 保存上下文到全局管理器
		ImGuiContextManager::SetContext(ImGui::GetCurrentContext());
		...
		io.Fonts->AddFontFromFileTTF("Resources/fonts/SiYuanHeiTiRegular/SourceHanSansCN-Bold.otf", fontSize, NULL, io.Fonts->GetGlyphRangesChineseSimplifiedCommon());
		io.FontDefault = io.Fonts->AddFontFromFileTTF("Resources/fonts/SiYuanHeiTiRegular/SourceHanSansCN-Regular.otf", fontSize, NULL, io.Fonts->GetGlyphRangesChineseSimplifiedCommon());
	...
	void ImGuiLayer::End()
	{
		...
		if (io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable)
		{
			...
			GLFWwindow* new_current = glfwGetCurrentContext();
			if (new_current != backup_current_context)
